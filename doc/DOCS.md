...
1. **``Fullscreen`` - меняет разрешение экрана в скрипте на ``полноэкранный``, либо на ``1146x542``**


2. **``Theme`` - смена темы на ``светлую`` или ``темную``**


3. **``Folder`` - открывает ту папку, где находится ``HidlowToolsGUI.py`` (main)**


4. **``Clear cache`` - очищает ``.pyc`` и ``__pycache__`` во всей папке внутри проекта**
