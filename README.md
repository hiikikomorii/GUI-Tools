...
![bg_main_png](assets/bgblack_2.png)

***
[Функции](#функции)  ||  [Boottraper](#boottraper) || [Console](#console) || [Доп.Функционал](#дополнительный-функционал) || [Установка](#установка) || [Обновления](CHANGELOG.md)
***
...
### Проект ``HidlowToolsGUI`` - это небольшой сборник утилит обёрнутый графический интерфейс. В нем имеется более ``15 функций``
***
### Функции
...
**11. ``Monitoring`` - выводит на экран % загруженности ``RAM``, ``CPU`` и ``актуальное время``. Работает по принципу ``диспечера задач``**
...
**Имеется несколько тем: ``blue (default)``, ``black``, ``white``**

![bsod_theme](assets/bsod_preview_black.png)


![bsod_theme](assets/bsod_preview_white.png)

**Имеется кнопка ``fullscreen`` для включения и выключения полноэкранного режима**

**Также в BootTraper в правом нижнем углу показывается дата и время запуска, и информация о python ``version``, ``build`` и ``compiler``**
***
...
3. **``Folder`` - нажав на кнопку, вас переместит в ``текущую директорию`` с HidlowTools.py**
...
**после чего запускаем ``boottraper``:
